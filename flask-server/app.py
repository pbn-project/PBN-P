...
def get_data():
    ...
    fontSize = request.json.get('fontSize', 200)
    frames = request.json.get('frames', 25)
    formatX = request.json.get('formatX', 800)
    formatY = request.json.get('formatY', 600)

    splitOptionYes = request.json.get('splitOptionYes', "true")
    splitDistance = request.json.get('splitDistance', 40)

    colorBGR = request.json.get('colorBGR', 241)
    colorBGG = request.json.get('colorBGG', 241)
    colorBGB = request.json.get('colorBGB', 241)

    color1R = request.json.get('color1R', 188)
    color1G = request.json.get('color1G', 238)
    color1B = request.json.get('color1B', 52)

    color2R = request.json.get('color2R', 168)
    color2G = request.json.get('color2G', 191)
    color2B = request.json.get('color2B', 171)

    color3R = request.json.get('color3R', 61)
    color3G = request.json.get('color3G', 52)
    color3B = request.json.get('color3B', 228)

    color4R = request.json.get('color4R', 252)
    color4G = request.json.get('color4G', 113)
    color4B = request.json.get('color4B', 214)
    ...
    drawBot.newDrawing()
    FRAMES = int(f"{frames}")
    FRAMES_MOVE = int(FRAMES/3)
    FRAMEPAUSE = int(FRAMES/5)

    CANVASW = int(f"{formatX}")
    CANVASH = int(f"{formatY}")
    ...
    SPLITLETTER = bool(f"{splitOptionYes}")
    # NOSPLITLETTER = bool(f"{splitOptionNo}")
    DOUBLESPLIT = False
    PAUSE = True
    if SPLITLETTER == True:
        PAUSE == False
    # if NOSPLITLETTER == True:
    #    SPLITLETTER = False
    XX = 0
    XX_1 = XX
    XX_2 = XX
    MOVE_X = int(f"{splitDistance}")
    ...
    STROKEWIDTH_2 = 0.5
    # colors
    RBC = float(f"{colorBGR}")
    GBC = float(f"{colorBGG}")
    BBC = float(f"{colorBGB}")

    RBC1 = float(f"{color1R}")
    GBC1 = float(f"{color1G}")
    BBC1 = float(f"{color1B}")

    RBC2 = float(f"{color2R}")
    GBC2 = float(f"{color2G}")
    BBC2 = float(f"{color2B}")

    RBC3 = float(f"{color3R}")
    GBC3 = float(f"{color3G}")
    BBC3 = float(f"{color3B}")

    RBC4 = float(f"{color4R}")
    GBC4 = float(f"{color4G}")
    BBC4 = float(f"{color4B}")
...
def hello_3():
    ...
    return {"image_string": image_string, "message": message}
